...
public class RoutineState extends State{
	...
	final Button[] allGames;
	final Boolean[] gamePlayable;
	private int firstGame; // first game on menu
	private int yChange;
	...
	public RoutineState(){
		...
		g0 = new Button(9, "Routine Game 0", Gdx.graphics.getWidth() / 2, 5 * Gdx.graphics.getHeight() / 6);
		g1 = new Button(9, "Routine Game 1", Gdx.graphics.getWidth() / 2, 4 * Gdx.graphics.getHeight() / 6);
		g2 = new Button(9, "Routine Game 2", Gdx.graphics.getWidth() / 2, 3 * Gdx.graphics.getHeight() / 6);
		g3 = new Button(9, "Routine Game 3", Gdx.graphics.getWidth() / 2, 2 * Gdx.graphics.getHeight() / 6);
	...
	public void update() {
		...
		if(Gdx.input.isTouched())
		{
			...
			if (Gdx.input.getDeltaY() < -7 && firstGame < 5) {
				// up swipe
				firstGame++;
				yChange = 5;
				for (int i = firstGame; i < firstGame+5; i++) {
